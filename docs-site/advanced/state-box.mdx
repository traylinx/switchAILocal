---
title: State Box
description: Secure, centralized state management with automatic permission hardening
icon: box
---
...
Discovery cache entries follow the same rule. A cache entry can persist a
dynamically configured source URL, so it is treated as sensitive even when the
current source has no credentials. Model registries contain only provider and
capability inventory, but remain `0600` for one auditable State Box invariant.
Discovery cache and registry writes prefer atomic replacement, which also
migrates files created by older versions with broader permissions; filesystems
that reject replacement fall back to chmod-before-truncate. Existing credential
and config targets use chmod-before-truncate directly so bind mounts, symlinks,
hard links, and read-only parent directories preserve inode semantics. The file
owner must still be allowed to apply `0600`; cross-UID mounts that reject chmod
fail closed rather than keeping a broad mode. Only the switchAILocal-owned
discovery/auth leaf is tightened, on a best-effort basis; a custom parent keeps
its operator-selected mode. These numeric mode guarantees apply to supported
POSIX release targets (Linux and macOS).
...
<Accordion title="Perfect for Immutable Deployments">
...
1. Write → /tmp/registry.json.tmp.a1b2c3d4
2. fsync() → Flush to disk
3. Rename → /tmp/registry.json.tmp.a1b2c3d4 → registry.json
4. Backup → registry.json → registry.json.bak
```

```text On Power Failure
Scenario: Power failure during step 1 or 2

Result:
- Original registry.json intact
- Temporary file incomplete (ignored)
...
- [Configuration Guide](/configuration/overview) - Complete config.yaml options
- [Management Dashboard](/guides/guides/management-dashboard) - Web UI for state management
