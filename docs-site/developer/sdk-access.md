...
The `github.com/traylinx/switchAILocal/sdk/access` package centralizes inbound request authentication for the proxy. It offers a lightweight manager that chains credential providers, so servers can reuse the same access control logic inside or outside the CLI runtime.
...
Add the module with `go get github.com/traylinx/switchAILocal/sdk/access`.
...
## Integration with switchailocal Service

`sdk/switchailocal` wires `@sdk/access` automatically when you build a CLI service via `switchailocal.NewBuilder`. Supplying a preconfigured manager allows you to extend or override the default providers:
...
This mirrors the behaviour in `switchailocal.Service.refreshAccessProviders` and `api.Server.applyAccessConfig`, enabling runtime updates without restarting the process.
