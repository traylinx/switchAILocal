...
Use the `auto` or `auto:<intent>` model prefix to activate the Cortex Router. It evaluates all available providers using a composite scoring algorithm and selects the best one.

```bash
# Basic: Let Cortex pick the best model
curl http://localhost:18080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer sk-test-123" \
  -d '{"model": "auto", "messages": [{"role": "user", "content": "Hello!"}]}'

# Intent-based: Route to coding-optimized models
curl http://localhost:18080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer sk-test-123" \
...
Each candidate model is scored using configurable weights:

```
BaseScore    = W_a × Availability + W_q × Quota + W_l × Latency + W_s × SuccessRate
TierScore    = BaseScore + TierBoost(provider_tier)
PrefScore    = TierScore + PreferenceBoost(model)
FinalScore   = PrefScore × ConservationMultiplier(tier, complexity)
```

The model with the highest `FinalScore` wins. Configure weights in `config.yaml`:
...
When enabled, simple requests (short prompts below `simple-threshold-tokens`) automatically penalize expensive premium-tier models, routing to cheaper alternatives:
...
The Lab runs shadow experiments to continuously improve routing quality:

1. **Hypothesis**: Generate slightly perturbed weights
2. **Shadow Score**: Score every request with both production and shadow weights
3. **Observe**: Accumulate RQS (Routing Quality Score) over the adaptation interval
4. **Promote/Discard**: If shadow weights produce ≥5% higher RQS, promote them
...
Define which models are eligible for specific intents:
...
```bash
# Current Lab status + live weights
curl http://localhost:18080/v0/management/autoroute/status

# Recent routing decisions journal
curl http://localhost:18080/v0/management/autoroute/journal
```

Or view the dashboard at `http://localhost:18080/management`.

### Legacy Provider-Priority Routing

You can also route by simply omitting the provider prefix from your model name:
