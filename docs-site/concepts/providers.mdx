...
- Music generation + covers: `/music/generations` — for MiniMax, adapter in `internal/runtime/executor/minimax_music.go` routes to upstream `/v1/music_generation`, hex-decodes the audio, and returns `{data: {audio: <base64>, duration_ms, sample_rate, channels, bitrate}}`.
