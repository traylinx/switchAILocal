...
- Audio speech: `/audio/speech` — for MiniMax, OpenAI-shape or MiniMax-native requests are translated to native `/t2a_v2` by an adapter in `internal/runtime/executor/minimax_tts.go`. MiniMax error codes (1002/2013/2061) are mapped onto HTTP status so the failover taxonomy can classify them correctly.
