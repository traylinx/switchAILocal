...
The examples use Go 1.24+ and the `github.com/traylinx/switchAILocal` module.
...
svc, _ := switchailocal.NewBuilder().WithConfig(cfg).WithConfigPath(cfgPath).WithCoreAuthManager(core).Build()
```
...
switchailocal.GlobalModelRegistry().RegisterClient(authID, "myprov", models)
```
...
- Enable request logging: Management API GET/PUT/PATCH `/v0/management/request-log`
- Toggle debug logs: Management API GET/PUT/PATCH `/v0/management/debug`
