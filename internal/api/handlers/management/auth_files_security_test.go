...
func TestUploadAuthFile_PathTraversal(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:           "Valid File",
			queryName:      "valid.json",
			expectedStatus: http.StatusInternalServerError, // Fails due to unmarshaling empty JSON data from mock after passing path check
		},
		{
			name:           "Path Traversal Attempt",
			queryName:      "../secrets/secret.json",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			req, _ := http.NewRequest("POST", "/upload?name="+tc.queryName, http.NoBody)
			w := httptest.NewRecorder()
			r.ServeHTTP(w, req)
...
func TestDeleteAuthFile_PathTraversal(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:           "Valid File",
			queryName:      "valid.json",
			expectedStatus: http.StatusNotFound, // Passes path check, but file does not exist to be deleted in mock env
		},
		{
			name:           "Path Traversal Attempt",
			queryName:      "../secrets/secret.json",
