...
func TestUploadAuthFile_PathTraversal(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:           "Valid File",
			queryName:      "valid.json",
			expectedStatus: http.StatusServiceUnavailable, // Will fail authManager check but pass traversal check
		},
		{
			name:           "Path Traversal Attempt",
			queryName:      "../secrets/secret.json",
...
func TestDeleteAuthFile_PathTraversal(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:           "Path Traversal Attempt",
			queryName:      "../secrets/secret.json",
			expectedStatus: http.StatusBadRequest, // Depends on where the auth manager check is
		},
		{
			name:           "Path Traversal with Backslash (Windows style)",
			queryName:      "..\\secrets\\secret.json",
