...
func (h *Handler) GetSwitchAIKeys(c *gin.Context) {
	c.JSON(200, gin.H{"switchai-api-key": h.cfg.SwitchAIKey})
}
...
func (h *Handler) PutSwitchAIKeys(c *gin.Context) {
	...
	var arr []config.SwitchAIKey
	if err = json.Unmarshal(data, &arr); err != nil {
		var obj struct {
			Items []config.SwitchAIKey `json:"items"`
		}
		if err2 := json.Unmarshal(data, &obj); err2 != nil || len(obj.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
	...
	for i := range arr {
		normalizeSwitchAIKey(&arr[i])
	}
	h.cfg.SwitchAIKey = arr
	h.cfg.SanitizeSwitchAIKeys()
	h.persist(c)
}

func (h *Handler) PatchSwitchAIKey(c *gin.Context) {
	type switchAIKeyPatch struct {
		APIKey         *string                 `json:"api-key"`
		Prefix         *string                 `json:"prefix"`
		BaseURL        *string                 `json:"base-url"`
		ProxyURL       *string                 `json:"proxy-url"`
		Models         *[]config.SwitchAIModel `json:"models"`
		Headers        *map[string]string      `json:"headers"`
		ExcludedModels *[]string               `json:"excluded-models"`
	}
	var body struct {
		Index *int              `json:"index"`
		Match *string           `json:"match"`
		Value *switchAIKeyPatch `json:"value"`
	}
	if err := c.ShouldBindJSON(&body); err != nil || body.Value == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	...
	targetIndex := -1
	if body.Index != nil && *body.Index >= 0 && *body.Index < len(h.cfg.SwitchAIKey) {
		targetIndex = *body.Index
	}
	if targetIndex == -1 && body.Match != nil {
		match := strings.TrimSpace(*body.Match)
		for i := range h.cfg.SwitchAIKey {
			if h.cfg.SwitchAIKey[i].APIKey == match {
				targetIndex = i
				break
			}
		}
	...
	entry := h.cfg.SwitchAIKey[targetIndex]
	if body.Value.APIKey != nil {
		entry.APIKey = strings.TrimSpace(*body.Value.APIKey)
	}
	if body.Value.Prefix != nil {
	...
	if body.Value.Models != nil {
		entry.Models = append([]config.SwitchAIModel(nil), (*body.Value.Models)...)
	}
	if body.Value.Headers != nil {
		entry.Headers = config.NormalizeHeaders(*body.Value.Headers)
	}
	...
	normalizeSwitchAIKey(&entry)
	h.cfg.SwitchAIKey[targetIndex] = entry
	h.cfg.SanitizeSwitchAIKeys()
	h.persist(c)
}
...
func (h *Handler) DeleteSwitchAIKey(c *gin.Context) {
	if val := c.Query("api-key"); val != "" {
		out := make([]config.SwitchAIKey, 0, len(h.cfg.SwitchAIKey))
		for _, v := range h.cfg.SwitchAIKey {
			if v.APIKey != val {
				out = append(out, v)
			}
		}
		h.cfg.SwitchAIKey = out
		h.cfg.SanitizeSwitchAIKeys()
		h.persist(c)
		return
	}
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		_, err := fmt.Sscanf(idxStr, "%d", &idx)
		if err == nil && idx >= 0 && idx < len(h.cfg.SwitchAIKey) {
			h.cfg.SwitchAIKey = append(h.cfg.SwitchAIKey[:idx], h.cfg.SwitchAIKey[idx+1:]...)
			h.cfg.SanitizeSwitchAIKeys()
			h.persist(c)
			return
		}
...
func normalizeSwitchAIKey(entry *config.SwitchAIKey) {
	if entry == nil {
		return
	}
	entry.APIKey = strings.TrimSpace(entry.APIKey)
	...
	normalized := make([]config.SwitchAIModel, 0, len(entry.Models))
	for i := range entry.Models {
		model := entry.Models[i]
		model.Name = strings.TrimSpace(model.Name)
		model.Alias = strings.TrimSpace(model.Alias)
