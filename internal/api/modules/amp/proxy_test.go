...
func TestModifyResponse_UpdatesContentLengthHeader(t *testing.T) {
	...
	got, errReadAll := io.ReadAll(resp.Body)
	if errReadAll != nil {
		t.Fatalf("ReadAll error: %v", errReadAll)
	}
...
func TestReverseProxy_FullRoundTrip_Gzip(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
		if _, err := w.Write(gzipBytes([]byte(`{"upstream":"ok"}`))); err != nil {
			t.Errorf("failed to write response: %v", err)
		}
	}))
	defer upstream.Close()
...
func TestReverseProxy_FullRoundTrip_PlainJSON(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(200)
		if _, err := w.Write([]byte(`{"plain":"json"}`)); err != nil {
			t.Errorf("failed to write response: %v", err)
		}
	}))
	defer upstream.Close()
