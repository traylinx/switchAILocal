...
import (
	...
	"github.com/traylinx/switchAILocal/internal/access"
	managementHandlers "github.com/traylinx/switchAILocal/internal/api/handlers/management"
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, luaEngine *plugin.LuaEngine, opts ...ServerOption) *Server {
	...
	if !cfg.WebsocketAuth {
		log.Warn("SECURITY WARNING: WebSocket authentication is DISABLED. This allows unauthenticated access to the relay.")
	}
	...
	// Authentication and localhost bypass are handled by the management middleware.
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/metrics", s.mgmt.GetSuperbrainMetrics)
		...
		mgmt.GET("/switchai-api-key", s.mgmt.GetSwitchAIKeys)
		mgmt.PUT("/switchai-api-key", s.mgmt.PutSwitchAIKeys)
		mgmt.PATCH("/switchai-api-key", s.mgmt.PatchSwitchAIKey)
		mgmt.DELETE("/switchai-api-key", s.mgmt.DeleteSwitchAIKey)
		...
		mgmt.POST("/providers/test", s.mgmt.TestProvider)
		mgmt.POST("/providers/discover-models", s.mgmt.DiscoverModels)
...
// providersHandler returns a handler for /v1/providers endpoint
// that lists all available AI providers with their model counts and status.
// Supports query param ?filter=active|inactive|all (default: all)
func (s *Server) providersHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if filter != "" && filter != "all" {
			var filtered []registry.ProviderInfo
...
// refreshModelsHandler handles POST /v1/models/refresh to trigger model re-discovery.
func (s *Server) refreshModelsHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		if s.discoverer == nil {
			c.JSON(http.StatusServiceUnavailable, gin.H{
				"error": "Model discovery not initialized",
			})
			return
		}
		...
		if err := s.discoverer.Refresh(ctx, providerID); err != nil {
			log.WithError(err).Warn("Model refresh failed")
		...
		// Update registry with newly discovered models
		results, discErr := s.discoverer.DiscoverAll(ctx)
		if discErr == nil {
			modelReg := registry.GetGlobalRegistry()
			for prov, models := range results {
				// Use the original discovery provider IDs to keep CLI and API separate
				regProvider := prov
				modelReg.RegisterClient("discovery-"+prov, regProvider, models)
			}
		}
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	// Ensure management routes are registered and enabled.
	s.registerManagementRoutes()
	s.managementRoutesEnabled.Store(true)
