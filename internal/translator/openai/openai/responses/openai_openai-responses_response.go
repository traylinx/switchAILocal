...
func emitRespEvent(event string, payload string) string {
	// Optimize: Use direct string concatenation instead of fmt.Sprintf to eliminate reflection overhead in SSE hot paths
	return "event: " + event + "\ndata: " + payload
}
			...
			if delta.Exists() {
				...
				if tcs := delta.Get("tool_calls"); tcs.Exists() && tcs.IsArray() {
					...
					tcs.ForEach(func(_, tc gjson.Result) bool {
						tcIdxStr := tc.Get("index").String()
						if tcIdxStr == "" {
							return true
						}
						tcIdx := int(tc.Get("index").Int())
						stIdx := 1000 + tcIdx // namespace to avoid collision with message output_index

						newCallID := tc.Get("id").String()
						nameChunk := tc.Get("function.name").String()
						if nameChunk != "" {
							st.FuncNames[stIdx] = nameChunk
						}
						existingCallID := st.FuncCallIDs[stIdx]
						...
						if existingCallID == "" && newCallID != "" {
							...
							st.FuncCallIDs[stIdx] = newCallID
							shouldEmitItem = true
						}
						...
						if st.FuncArgsBuf[stIdx] == nil {
							st.FuncArgsBuf[stIdx] = &strings.Builder{}
						}
						...
						if args := tc.Get("function.arguments"); args.Exists() && args.String() != "" {
							...
							refCallID := st.FuncCallIDs[stIdx]
							if refCallID == "" {
								refCallID = newCallID
							}
							if refCallID != "" {
							...
							st.FuncArgsBuf[stIdx].WriteString(args.String())
						}
						return true
					})
				}
