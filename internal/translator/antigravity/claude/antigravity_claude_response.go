...
func appendFinalEvents(params *Params, output *string, force bool) {
	...
	*output = *output + "data: "
	delta := `{"type":"message_delta","delta":{"stop_reason":"` + stopReason + `","stop_sequence":null},"usage":{"input_tokens":` + strconv.FormatInt(params.PromptTokenCount, 10) + `,"output_tokens":` + strconv.FormatInt(usageOutputTokens, 10) + `}}`
