...
func ConvertClaudeResponseToGeminiNonStream(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) string {
	...
	// Use a manual loop with bytes.IndexByte to avoid the large 50MB allocation per request
	// that a bufio.Scanner requires to handle the potentially enormous Claude SSE events
	data := rawJSON
	for len(data) > 0 {
