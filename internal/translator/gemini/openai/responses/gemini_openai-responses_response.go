...
func (st *geminiToResponsesState) ensureFuncCapacity(idx int) {
	if idx >= len(st.FuncArgsBuf) {
		// Calculate new capacity (at least idx + 1)
		// We can grow slightly more to avoid frequent reallocations, e.g., next power of 2 or chunk size
		newCap := idx + 1
		if newCap < 2*len(st.FuncArgsBuf) {
			newCap = 2 * len(st.FuncArgsBuf)
		}
		if newCap < 4 {
			newCap = 4
		}
