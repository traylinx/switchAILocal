...
func (st *geminiToResponsesState) ensureFuncCapacity(idx int) {
	if idx >= len(st.FuncArgsBuf) {
		newLen := idx + 1
		grow := newLen - len(st.FuncArgsBuf)
		st.FuncArgsBuf = append(st.FuncArgsBuf, make([]*strings.Builder, grow)...)
		st.FuncNames = append(st.FuncNames, make([]string, grow)...)
		st.FuncCallIDs = append(st.FuncCallIDs, make([]string, grow)...)
	}
}
