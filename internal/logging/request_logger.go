...
// Package logging provides request logging functionality for the switchAILocal server.
...
var (
	...
	// invalidFilenameChars matches characters that are not safe/allowed in filenames: < > : " | ? * and whitespace.
...
func (l *FileRequestLogger) sanitizeForFilename(path string) string {
	...
	sanitized = invalidFilenameChars.ReplaceAllString(sanitized, "-")
...
func (w *FileStreamingLogWriter) WriteChunkAsync(chunk []byte) {
	...
	if cap(*pooledBuf) < len(chunk) {
		*pooledBuf = make([]byte, len(chunk))
	} else {
		*pooledBuf = (*pooledBuf)[:len(chunk)]
	}
	copy(*pooledBuf, chunk)
	...
	default:
		...
		// Recycle buffer to prevent memory leaks
