...
func TestSyncAuthFromDatabase(t *testing.T) {
	...
	for _, id := range []string{"auth1.json", "nested/auth2.json"} {
		if _, err := os.Stat(filepath.Join(store.authDir, filepath.FromSlash(id))); err != nil {
			t.Errorf("expected %s: %v", id, err)
		}
	}
	files := postgresAuthTreeFiles(t, store.authDir)
	if got, want := fmt.Sprint(files), fmt.Sprint([]string{"auth1.json", "nested/auth2.json"}); got != want {
		t.Fatalf("sync files = %s, want %s", got, want)
	}
}
...
func TestSyncAuthFromDatabase_Pagination(t *testing.T) {
	...
	mock.ExpectQuery(query2).WithArgs("auth100.json").WillReturnRows(rows2)
