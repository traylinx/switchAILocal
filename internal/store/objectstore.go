...
func (s *ObjectTokenStore) Save(ctx context.Context, auth *switchailocalauth.Auth) (string, error) {
	if auth == nil {
...
func (s *ObjectTokenStore) List(_ context.Context) ([]*switchailocalauth.Auth, error) {
	dir := strings.TrimSpace(s.AuthDir())
	if dir == "" {
		return nil, fmt.Errorf("object store: auth directory not configured")
	}
	entries := make([]*switchailocalauth.Auth, 0, 32)
...
func (s *ObjectTokenStore) resolveAuthPath(auth *switchailocalauth.Auth) (string, error) {
	if auth == nil {
		return "", fmt.Errorf("object store: auth is nil")
	}
	if auth.Attributes != nil {
...
func (s *ObjectTokenStore) readAuthFile(path, baseDir string) (*switchailocalauth.Auth, error) {
	data, err := os.ReadFile(path)
	if err != nil {
