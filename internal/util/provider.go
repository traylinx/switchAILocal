...
var (
	...
	// sensitiveFormKeys matches key=value patterns for sensitive terms.
	// It captures up to the next ampersand or end of string.
	// Used for form-url-encoded data or query params in bodies.
	sensitiveFormKeys = regexp.MustCompile(`(?i)(api_?key|apikey|token|secret|password|authorization)=([^&]*)`)
)

// MaskSensitiveBody masks sensitive fields in a body (JSON or Form).
// It uses regex to identify and mask values of sensitive keys while preserving structure.
func MaskSensitiveBody(body []byte) []byte {
	...
	// 1. Mask JSON fields
	body = sensitiveJSONKeys.ReplaceAllFunc(body, func(match []byte) []byte {
	...
	// 2. Mask Form fields
	body = sensitiveFormKeys.ReplaceAllFunc(body, func(match []byte) []byte {
		// match contains the full key=value string
		submatches := sensitiveFormKeys.FindSubmatchIndex(match)
		if len(submatches) < 6 {
			return match
		}
...
// Deprecated: Use MaskSensitiveBody instead.
// MaskSensitiveJSONBody is kept for backward compatibility if any external callers exist.
