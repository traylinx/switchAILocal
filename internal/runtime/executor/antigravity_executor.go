...
func (e *AntigravityExecutor) Execute(ctx context.Context, auth *switchailocalauth.Auth, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (resp switchailocalexecutor.Response, err error) {
	...
	for idx, baseURL := range baseURLs {
		...
		resp = switchailocalexecutor.Response{Payload: []byte(converted)}
...
func (e *AntigravityExecutor) executeClaudeNonStream(ctx context.Context, auth *switchailocalauth.Auth, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (resp switchailocalexecutor.Response, err error) {
	token, updatedAuth, errToken := e.ensureAccessToken(ctx, auth)
	if errToken != nil {
		return resp, errToken
	}
	...
	for idx, baseURL := range baseURLs {
		...
		resp = switchailocalexecutor.Response{Payload: e.convertStreamToNonStream(buffer.Bytes())}
...
func (e *AntigravityExecutor) CountTokens(ctx context.Context, auth *switchailocalauth.Auth, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (switchailocalexecutor.Response, error) {
	token, updatedAuth, errToken := e.ensureAccessToken(ctx, auth)
	if errToken != nil {
		return switchailocalexecutor.Response{}, errToken
	}
	if updatedAuth != nil {
		auth = updatedAuth
	}
	if strings.TrimSpace(token) == "" {
		return switchailocalexecutor.Response{}, statusErr{code: http.StatusUnauthorized, msg: "missing access token"}
	}
	...
	for idx, baseURL := range baseURLs {
		...
		return switchailocalexecutor.Response{}, statusErr{code: httpResp.StatusCode, msg: string(bodyBytes)}
	}

	switch {
	case lastStatus != 0:
		return switchailocalexecutor.Response{}, statusErr{code: lastStatus, msg: string(lastBody)}
	case lastErr != nil:
		return switchailocalexecutor.Response{}, lastErr
	default:
		return switchailocalexecutor.Response{}, statusErr{code: http.StatusServiceUnavailable, msg: "antigravity executor: no base url available"}
	}
}
...
func (e *AntigravityExecutor) ensureAccessToken(ctx context.Context, auth *switchailocalauth.Auth) (string, *switchailocalauth.Auth, error) {
	if auth == nil {
...
func (e *AntigravityExecutor) refreshToken(ctx context.Context, auth *switchailocalauth.Auth) (*switchailocalauth.Auth, error) {
	if auth == nil {
		return nil, statusErr{code: http.StatusUnauthorized, msg: "missing auth"}
	}
...
func (e *AntigravityExecutor) buildRequest(ctx context.Context, auth *switchailocalauth.Auth, token, modelName string, payload []byte, stream bool, alt, baseURL string) (*http.Request, error) {
	if token == "" {
...
func buildBaseURL(auth *switchailocalauth.Auth) string {
...
func resolveUserAgent(auth *switchailocalauth.Auth) string {
	if auth != nil {
...
func antigravityBaseURLFallbackOrder(auth *switchailocalauth.Auth) []string {
...
func resolveCustomAntigravityBaseURL(auth *switchailocalauth.Auth) string {
	if auth == nil {
		return ""
	}
	if auth.Attributes != nil {
