...
func newUsageReporter(ctx context.Context, provider, model string, auth *switchailocalauth.Auth) *usageReporter {
...
func resolveUsageSource(auth *switchailocalauth.Auth, ctxAPIKey string) string {
	if auth != nil {
		provider := strings.TrimSpace(auth.Provider)
		if strings.EqualFold(provider, constant.GeminiCLI) ||
			strings.EqualFold(provider, constant.ClaudeCLI) ||
			strings.EqualFold(provider, constant.VibeCLI) {
