...
func (e *LocalCLIExecutor) executeRemote(ctx context.Context, remoteHost, binary string, args []string, modelName string) (switchailocalexecutor.Response, error) {
	...
	// Validate URL scheme to prevent SSRF (Server-Side Request Forgery)
	// Ensuring scheme is explicitly http or https prevents exploitation
	// via arbitrary URL schemes like file:// or gopher://
	parsedURL, err := url.Parse(rawurl)
	if err != nil {
		return switchailocalexecutor.Response{}, fmt.Errorf("invalid remote bridge URL: %w", err)
	}
	if parsedURL.Scheme != "http" && parsedURL.Scheme != "https" {
		return switchailocalexecutor.Response{}, fmt.Errorf("invalid remote bridge URL scheme: %s", parsedURL.Scheme)
	}
