...
func (e *LocalCLIExecutor) executeRemote(ctx context.Context, remoteHost, binary string, args []string, modelName string) (switchailocalexecutor.Response, error) {
	...
	// Validate remote host URL to prevent SSRF scheme abuse
	...
	if parsedURL.Scheme != "http" && parsedURL.Scheme != "https" {
		return switchailocalexecutor.Response{}, fmt.Errorf("invalid URL scheme %q: only http and https are allowed to prevent SSRF", parsedURL.Scheme)
	}
