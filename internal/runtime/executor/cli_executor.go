...
func (e *LocalCLIExecutor) executeRemote(ctx context.Context, remoteHost, binary string, args []string, modelName string) (switchailocalexecutor.Response, error) {
	...
	// Mitigate SSRF by enforcing strict protocol validation.
	// Use rawurl to avoid shadowing the "net/url" package.
	rawurl := strings.TrimSuffix(remoteHost, "/") + "/run"
	parsedURL, err := url.Parse(rawurl)
	if err != nil {
		return switchailocalexecutor.Response{}, fmt.Errorf("invalid bridge URL: %w", err)
	}
	if parsedURL.Scheme != "http" && parsedURL.Scheme != "https" {
		return switchailocalexecutor.Response{}, fmt.Errorf("invalid bridge URL scheme: %s. Only http and https are allowed", parsedURL.Scheme)
	}
