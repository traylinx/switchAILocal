...
func (e *QwenExecutor) PrepareRequest(_ *http.Request, _ *switchailocalauth.Auth) error { return nil }
...
func (e *QwenExecutor) ExecuteStream(ctx context.Context, auth *switchailocalauth.Auth, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (stream <-chan switchailocalexecutor.StreamChunk, err error) {
	...
	go func() {
		...
		for i := range doneChunks {
			if !sendStreamChunk(ctx, out, switchailocalexecutor.StreamChunk{Payload: []byte(doneChunks[i])}) {
				return
			}
		}
...
func (e *QwenExecutor) CountTokens(ctx context.Context, auth *switchailocalauth.Auth, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (switchailocalexecutor.Response, error) {
	...
	if err != nil {
		return switchailocalexecutor.Response{}, fmt.Errorf("qwen executor: tokenizer init failed: %w", err)
	}

	count, err := countOpenAIChatTokens(enc, body)
	if err != nil {
		return switchailocalexecutor.Response{}, fmt.Errorf("qwen executor: token counting failed: %w", err)
	}
...
func qwenCreds(a *switchailocalauth.Auth) (token, baseURL string) {
	if a == nil {
		return "", ""
	}
	if a.Attributes != nil {
