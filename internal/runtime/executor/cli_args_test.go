...
func TestLocalCLIExecutor_BuildFinalArgs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			cliOpts:    nil,
			formatArgs: nil,
			wantArgs:   []string{"-y", "--include-directories=/", "--", "-dangerous-flag"},
		},
		{
			...
			cliOpts:    nil,
			formatArgs: nil,
			wantArgs:   []string{"-y", "--include-directories=/", "--", "hello world"},
		},
		{
		...
		},
		{
			...
			wantArgs: []string{"-s", "-y", "--include-directories=/", "--json", "--", "@test.txt analyze this"},
		},
		{
			...
			formatArgs: nil,
			wantArgs:   []string{"-s", "-y", "--include-directories=/", "--", "hello"},
		},
	}
