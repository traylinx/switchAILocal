...
func TestFileSynthesizer_Synthesize_ValidAuthFile(t *testing.T) {
	...
	if auths[0].Provider != "claudecli" {
		t.Errorf("expected provider claudecli, got %s", auths[0].Provider)
	}
...
func TestFileSynthesizer_Synthesize_GeminiProviderMapping(t *testing.T) {
	...
	if auths[0].Provider != "geminicli" {
		t.Errorf("gemini should be mapped to geminicli, got %s", auths[0].Provider)
	}
}
...
func TestSynthesizeGeminiVirtualAuths_MultiProject(t *testing.T) {
	...
	for i, v := range virtuals {
		if v.Provider != "geminicli" {
			t.Errorf("expected provider geminicli, got %s", v.Provider)
		}
...
func TestSynthesizeGeminiVirtualAuths_EmptyProviderAndLabel(t *testing.T) {
	...
	// Check that empty provider defaults to geminicli
	if virtuals[0].Provider != "geminicli" {
		t.Errorf("expected provider geminicli (default), got %s", virtuals[0].Provider)
	}
	// Check that empty label defaults to provider
	if !strings.Contains(virtuals[0].Label, "geminicli") {
		t.Errorf("expected label to contain geminicli, got %s", virtuals[0].Label)
	}
}
