...
import (
	...
	"github.com/traylinx/switchAILocal/internal/watcher/synthesizer"
...
func TestBuildAPIKeyClientsCounts(t *testing.T) {
	...
	gemini, vertex, claude, codex, switchai, compat := BuildAPIKeyClients(cfg)
	if gemini != 2 || vertex != 1 || claude != 1 || codex != 2 || switchai != 0 || compat != 2 {
		t.Fatalf("unexpected counts: %d %d %d %d %d %d", gemini, vertex, claude, codex, switchai, compat)
	}
}
...
func TestSnapshotCoreAuths_ConfigAndAuthFiles(t *testing.T) {
	...
	if geminiPrimary == nil {
		t.Fatal("expected primary geminicli auth from file")
	}
	if !geminiPrimary.Disabled || geminiPrimary.Status != coreauth.StatusDisabled {
		t.Fatal("expected primary geminicli auth to be disabled when virtual auths are synthesized")
	}
