...
## 2026-08-22 - Fix SSRF vulnerability in CLI executor
**Vulnerability:** G704 (SSRF) was flagged in `/app/internal/runtime/executor/cli_executor.go` because the target URL was constructed by taking a parsed URL by value, modifying the `Path` and nullifying the `RawPath` and `Fragment`, and calling `.String()`.
**Learning:** If an attacker provides an opaque URL (where `url.Opaque` is set), the `url.URL.String()` method prioritizes the `Opaque` field over any manually modified `Path` field, allowing path constraints to be bypassed. Always construct a new `url.URL{}` struct literal explicitly when reconstructing URLs to mitigate SSRF.
**Prevention:** Avoid copying a parsed `url.URL` by value and manually nullifying fields. Instead, reconstruct URLs using a strict `url.URL{...}` struct literal, assigning only the needed fields explicitly.
