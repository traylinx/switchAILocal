...
## 2026-07-01 - Fix SSRF Vulnerability via Taint Analysis in executeRemote
**Vulnerability:** The `executeRemote` function within `cli_executor.go` blindly accepted the `REMOTE_COMMAND_HOST` environment variable as a raw URL and passed it directly to `http.NewRequestWithContext` and `http.DefaultClient.Do`. This resulted in a Server-Side Request Forgery (SSRF) vulnerability via taint analysis (G704), as an attacker could potentially manipulate the environment variable to point to unintended schemes (e.g., `file://`, `ftp://`) to access internal resources or local files.
**Learning:** Even for trusted, administrative configuration endpoints like `REMOTE_COMMAND_HOST`, strict protocol validation is mandatory. Relying on implicit HTTP/HTTPS assumptions allows scheme abuse (like `file://`). While restricting IP addresses to avoid private loops can introduce functional regressions for legitimate local bridge connections and TOCTOU vulnerabilities, validating the URL scheme provides robust defense-in-depth against protocol abuse without breaking functionality.
**Prevention:** Always parse environment variables or inputs representing URLs using `net/url.Parse` before passing them to HTTP clients. Explicitly validate that the `Scheme` is strictly `http` or `https`. Be aware that `gosec` taint analysis will likely still flag the `http.DefaultClient.Do` call as a residual false positive, but the explicit scheme validation safely mitigates the underlying SSRF protocol abuse risk.
