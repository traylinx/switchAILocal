...
## 2025-03-05 - 🛡️ Sentinel: Fix InsecureSkipVerify in Management Proxy Endpoints
**Vulnerability:** Found `InsecureSkipVerify: true` hardcoded within `TLSClientConfig` inside the HTTP Transport definitions in `internal/api/handlers/management/discover_models.go` and `internal/api/handlers/management/test_provider.go`. This allowed the application to silently bypass TLS certificate validation when making connections through proxies to downstream providers or discovery endpoints, exposing these management requests to Man-in-the-Middle (MITM) attacks.
**Learning:** The insecure flag was likely added for local developer convenience (as mentioned in a source code comment `// Allow insecure for testing/local proxies? Maybe strictly verifying is better but for devs...`) to allow traffic through local debugging proxies (like Charles or Burp) without configuring root certificates on the host machine. However, leaving this in production-facing API handlers handling sensitive requests defeats the primary defense against interception for AI proxy gateways.
**Prevention:** Never use `InsecureSkipVerify: true` in `TLSClientConfig` within `http.Transport` for API communication. Management handlers must strictly enforce standard TLS certificate validation without any insecure overrides, even if it adds friction for local debugging environments.
