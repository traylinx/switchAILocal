...
## 2026-08-08 - Prevent SSRF via URL Component Reconstruction
**Vulnerability:** Raw string concatenation used to build remote command URLs, allowing injection of paths or query parameters.
**Learning:** Parsing with `net/url` and verifying the scheme is insufficient if the target URL is reconstructed via string concatenation, bypassing host and path checks.
**Prevention:** Always reconstruct destination URLs safely using a strict `url.URL{}` struct literal, extracting only validated components.
