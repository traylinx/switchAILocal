...
## 2026-08-11 - Fix SSRF via taint analysis
**Vulnerability:** gosec G704 (SSRF via taint analysis) in cli_executor.go due to insecure URL reconstruction where `remoteHost` wasn't fully validated before sending a POST request to it.
**Learning:** Basic validation (checking scheme/host) isn't enough if the URL is then manipulated with raw string modifications (like appending `/run`) or if the initial parsed structure retains unvalidated components (like query params or fragments) that could be maliciously crafted to bypass the intent.
**Prevention:** Use `net/url` to explicitly reconstruct the URL using a strict `url.URL` struct literal, strictly assigning only validated components (Scheme, Host, specific Path) to prevent injection of malicious paths, query parameters, or host spoofing.
