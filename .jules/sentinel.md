...
## 2026-07-08 - Prevent SSRF and Open Redirect in Management and Execution Endpoints
...
## 2026-08-19 - [SSRF in Remote Execution Gateway]
**Vulnerability:** The `executeRemote` function in `cli_executor.go` constructed URLs via string manipulation (`targetURL.Path = strings.TrimSuffix(targetURL.Path, "/") + "/run"`) while modifying a dereferenced `url.URL` struct, which bypassed scheme validation by allowing downstream components to misinterpret the raw URL string, leading to an SSRF vulnerability flagged by gosec (G704).
**Learning:** Modifying path fields directly on a dereferenced `url.URL` and calling `.String()` can result in an inconsistent URL state if not carefully managed, exposing the application to SSRF if user-controlled input circumvents validation checks.
**Prevention:** When reconstructing URLs after validation, always build a new `url.URL{}` struct literal explicitly assigning only the validated components (Scheme, Host, Path, User, RawQuery) rather than modifying and reusing an existing parsed struct, ensuring no unvalidated parts leak into the final target URL.
