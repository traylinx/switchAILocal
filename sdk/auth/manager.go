...
func (m *Manager) Login(ctx context.Context, provider string, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, string, error) {
	...
	if !ok {
		return nil, "", fmt.Errorf("switchailocal auth: authenticator %s not registered", provider)
	}
	...
	if record == nil {
		return nil, "", fmt.Errorf("switchailocal auth: authenticator %s returned nil record", provider)
	}
