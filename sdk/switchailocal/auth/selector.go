...
func getAvailableAuths(auths []*Auth, provider, model string, now time.Time) ([]*Auth, error) {
	if len(auths) == 0 {
		return nil, &Error{
			Code:    "auth_not_found",
			Message: fmt.Sprintf("no auth candidates found for provider '%s'. To resolve this, please run: 'switchAILocal %s' or configure it in config.yaml.", provider, util.GetLoginHint(provider)),
		}
	}
	...
	if len(available) == 0 {
		...
		return nil, &Error{
			...
			Message: fmt.Sprintf("no active auth available for provider '%s' (all candidates are busy or cooling down). To resolve this, please run: 'switchAILocal %s' or configure additional credentials in config.yaml.", provider, util.GetLoginHint(provider)),
		}
	}
...
func (s *FillFirstSelector) Pick(ctx context.Context, provider, model string, opts switchailocalexecutor.Options, auths []*Auth) (*Auth, error) {
	_ = ctx
	_ = opts
	now := time.Now()
	available, err := getAvailableAuths(auths, provider, model, now)
