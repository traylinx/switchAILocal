...
type ProviderExecutor interface {
	...
	Execute(ctx context.Context, auth *Auth, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (switchailocalexecutor.Response, error)
	// ExecuteStream handles streaming execution and returns a channel of provider chunks.
	ExecuteStream(ctx context.Context, auth *Auth, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (<-chan switchailocalexecutor.StreamChunk, error)
	// Refresh attempts to refresh provider credentials and returns the updated auth state.
	Refresh(ctx context.Context, auth *Auth) (*Auth, error)
	// CountTokens returns the token count for the given request.
	CountTokens(ctx context.Context, auth *Auth, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (switchailocalexecutor.Response, error)
}
...
type Selector interface {
	Pick(ctx context.Context, provider, model string, opts switchailocalexecutor.Options, auths []*Auth) (*Auth, error)
}
...
func (m *Manager) Register(ctx context.Context, auth *Auth) (*Auth, error) {
	...
	// No ID check for now (allow overwrite?)
...
// DebugDump prints the current state of the auth map.
...
func (m *Manager) Execute(ctx context.Context, providers []string, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (switchailocalexecutor.Response, error) {
	...
	for attempt := 0; attempt < attempts; attempt++ {
		resp, errExec := m.executeProvidersOnce(ctx, rotated, func(execCtx context.Context, provider string) (switchailocalexecutor.Response, error) {
		...
		if errWait := waitForCooldown(ctx, wait); errWait != nil {
			return switchailocalexecutor.Response{}, errWait
		}
	}
	if lastErr != nil {
		return switchailocalexecutor.Response{}, lastErr
	}
	return switchailocalexecutor.Response{}, &Error{Code: "auth_not_found", Message: fmt.Sprintf("no active credentials available for the requested providers: %v. Please ensure you are logged in or have configured these providers in config.yaml.", rotated)}
}

// ExecuteCount performs a non-streaming execution using the configured selector and executor.
// It supports multiple providers for the same model and round-robins the starting provider per model.
func (m *Manager) ExecuteCount(ctx context.Context, providers []string, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (switchailocalexecutor.Response, error) {
	normalized := m.normalizeProviders(providers)
	if len(normalized) == 0 {
		return switchailocalexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
...
// It supports multiple providers for the same model and round-robins the starting provider per model.
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (<-chan switchailocalexecutor.StreamChunk, error) {
	normalized := m.normalizeProviders(providers)
	if len(normalized) == 0 {
	...
	var lastErr error
	for attempt := 0; attempt < attempts; attempt++ {
		chunks, errStream := m.executeStreamProvidersOnce(ctx, rotated, func(execCtx context.Context, provider string) (<-chan switchailocalexecutor.StreamChunk, error) {
	...
	return nil, &Error{Code: "auth_not_found", Message: fmt.Sprintf("no active credentials available for the requested providers: %v. Please ensure you are logged in or have configured these providers in config.yaml.", rotated)}
}

func (m *Manager) executeWithProvider(ctx context.Context, provider string, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (switchailocalexecutor.Response, error) {
	if provider == "" {
		return switchailocalexecutor.Response{}, &Error{Code: "provider_not_found", Message: "provider identifier is empty"}
	}
	...
	for {
		...
		if errPick != nil {
			...
			return switchailocalexecutor.Response{}, errPick
		}
		...
		if errExec != nil {
			...
			var se switchailocalexecutor.StatusError
		...
		resp.Metadata["executed_provider"] = provider
		return resp, nil
	}
}

func (m *Manager) executeCountWithProvider(ctx context.Context, provider string, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (switchailocalexecutor.Response, error) {
	if provider == "" {
		return switchailocalexecutor.Response{}, &Error{Code: "provider_not_found", Message: "provider identifier is empty"}
	}
	routeModel := req.Model
...
func (m *Manager) executeStreamWithProvider(ctx context.Context, provider string, req switchailocalexecutor.Request, opts switchailocalexecutor.Options) (<-chan switchailocalexecutor.StreamChunk, error) {
	if provider == "" {
	...
	for {
		...
		go func(streamCtx context.Context, streamAuth *Auth, streamProvider string, streamChunks <-chan switchailocalexecutor.StreamChunk) {
			defer close(out)
			var failed bool
...
func (m *Manager) executeProvidersOnce(ctx context.Context, providers []string, fn func(context.Context, string) (switchailocalexecutor.Response, error)) (switchailocalexecutor.Response, error) {
	if len(providers) == 0 {
		return switchailocalexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	var lastErr error
...
func (m *Manager) executeStreamProvidersOnce(ctx context.Context, providers []string, fn func(context.Context, string) (<-chan switchailocalexecutor.StreamChunk, error)) (<-chan switchailocalexecutor.StreamChunk, error) {
	...
	return nil, &Error{Code: "auth_not_found", Message: "no active credentials found for the requested providers. Please ensure you are logged in or have configured the providers in config.yaml."}
}
...
func (m *Manager) pickNext(ctx context.Context, provider, model string, opts switchailocalexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	for _, candidate := range m.auths {
		...
		if modelKey != "" && registryRef != nil {
			// Skip registry check for local CLI tools (they support what they support)
			isLocalCLI := false
			if src, ok := candidate.Metadata["source"].(string); ok && src == "local_cli_discovery" {
				isLocalCLI = true
			}

			if !isLocalCLI && !registryRef.ClientSupportsModel(candidate.ID, modelKey) {
				continue
			}
		}
