...
func (h *BaseAPIHandler) getRequestDetails(modelName string) (providers []string, normalizedModel string, metadata map[string]any, err *interfaces.ErrorMessage) {
	...
	// If explicit provider prefix was specified, validate it
	if providerPrefix != "" {
		// ALIAS MAPPING: Handle mistralcli -> vibe
		if providerPrefix == "mistralcli" {
			providerPrefix = "vibe"
		}

		actualModel, validationErr := util.ValidateProviderPrefix(providerPrefix, prefixedModel)
		if validationErr != nil {
			// Convert ProviderError to appropriate HTTP error
			status := http.StatusBadRequest
			if pe, ok := validationErr.(*util.ProviderError); ok {
				if pe.Type == util.ErrProviderNotConfigured {
		...
		if actualModel == "" {
			// Empty model after prefix (e.g., "ollama:") - use default/auto
			actualModel = "auto"
		}
