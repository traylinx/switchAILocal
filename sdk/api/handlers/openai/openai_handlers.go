...
func convertCompletionsRequestToChatCompletions(rawJSON []byte) []byte {
	...
	if req.Prompt != nil {
		...
		case []any:
			// Convert slice to JSON string representation to mimic gjson.String() behavior on arrays
			// Or better, just concatenate?
			// The original code used gjson.Get("prompt").String().
			// If prompt is ["a", "b"], gjson string is `["a", "b"]`.
			if len(v) > 0 {
				b, _ := json.Marshal(v)
				promptStr = string(b)
			}
	...
	chatReq.Messages[0].Content = promptStr
...
func convertChatCompletionsResponseToCompletions(rawJSON []byte) []byte {
	...
	if err := json.Unmarshal(rawJSON, &chatResp); err != nil {
		return rawJSON
	}
